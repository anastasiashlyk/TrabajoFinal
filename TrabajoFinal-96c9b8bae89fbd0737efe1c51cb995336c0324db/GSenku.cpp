#include <iostream>
#include <fstream>
#include <unistd.h>;libreria de unix en caso de querer hacer un sleep
#include "GSenku.hpp"
using namespace std;
...
int buscaSolucion(tpTablero &tablero, const tpMovimientosValidos &movValidos, tpListaMovimientos &solucionParcial, const int retardo){
  ...
  for(int i = 0; i < ocupadas.numPos; i++){
    ...
    for (int j = 0; j < posDestinos.numPos; j++){
      ...
      //if(retardo>0){ //usamos la funcion sleep para dejar de consumir CPU, entrando el kernel (funcion SO)
      //  mostrarTablero(tablero);
      //  sleep(retardo/1000);
      //}
      if(retardo>0){//implementacion de funcion de practicas con espera activa
          mostrarTablero(tablero);
          struct timespec inicio,fin;
          clock_gettime(CLOCK_REALTIME, &inicio);
          double tTrans=0;
          while(tTrans<retardo){
              clock_gettime(CLOCK_REALTIME, &fin);
              tTrans=(fin.tv_sec - inicio.tv_sec)*1000+((fin.tv_nsec - inicio.tv_nsec)/1000000);
          }
      }
      ...
      //if(retardo>0){
      //        cout << "deshago movimiento, vuelvo a:"<< endl;  
      //  mostrarTablero(tablero);
      //  sleep(retardo/1000);
      //}
